...
Use the Data Explorer to populate the `MaintenanceRequests` container with data from the `PropertyMaintenance.json` file provided by Arauco Rooms Demo.
...
4. In the new query window, paste in the following query:
    ...
    ```sql
    SELECT c.Salon_id, c.Salon, c.details, VectorDistance(c.request_vector, <QUERY_VECTOR>) AS SimilarityScore
    FROM c
    ```
